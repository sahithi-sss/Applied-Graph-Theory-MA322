...
def GS(m,n,pref_A, pref_B):
    free_A = list(range(m)) # all mentors are free initially
    current_matches = [-1] * n
    proposals = [set() for _ in range(m)]
    ...
    while free_A : #while there exists a free mentor (since set is made of mentors 
        ...
        for s in pref_A[m]: #traversing through the already order list of prefernces
            ...
            # Mentor has proposed to everyone → remove them
            free_A.pop(0)
    return current_matches

matches = GS(m,n,pref_A, pref_B)
